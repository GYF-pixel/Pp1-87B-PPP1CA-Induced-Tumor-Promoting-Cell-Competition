...
setwd("H:\\PPP1CA&PPP1CC\\08Bulk_RNAseq_FACS_PDX\\00Countingfiles")
...
{
  files <- all_files[i]
  model_path = paste('H:\\PPP1CA&PPP1CC\\08Bulk_RNAseq_FACS_PDX\\00Countingfiles\\',files, sep='')
  print(model_path)
}
...
newdata[62752:62759,]
newdata[62755:62759,]
newdata_filter <- newdata[1:62754,]
...
library(scales)
library(xlsx)
setwd("H:\\PPP1CA&PPP1CC\\08Bulk_RNAseq_FACS_PDX\\01Analysis")                    #设置工作目录 EyeDisc 替换为 WingDisc
...
exp=rt[,2:ncol(rt)]
data = exp
data.filter = data[rowMeans(data) > 10 & rowSums(data) > 15, ]   #数据过滤

# condition table
colnames(data.filter)
data.filter <- data.filter %>% select("X8988t.NT1.GFP","X8988t.NT3.GFP","X8988t.NT4.GFP",
"X8988t.NT1.mCherry","X8988t.NT4.mCherry","X8988t.NT5.mCherry",
"X8988t.PA1.GFP","X8988t.PA3.GFP",
"X8988t.PA1.mCherry","X8988t.PA2.mCherry","X8988t.PA3.mCherry")
colnames(data.filter)
group_info = c(rep("NT_GFP",3), rep("NT_mCherry",3), rep("PA_GFP",2), rep("PA_mCherry",3))

data.filter <- data.filter %>% select("X8988t.NT1.GFP","X8988t.NT3.GFP","X8988t.NT4.GFP",
"X8988t.PA1.mCherry","X8988t.PA2.mCherry","X8988t.PA3.mCherry")
group_info = c(rep("PA_GFP",2), rep("PA_mCherry",3))

data.filter <- data.filter %>% select("X8988t.NT1.GFP","X8988t.NT3.GFP","X8988t.NT4.GFP",
"X8988t.PA1.GFP","X8988t.PA3.GFP")
group_info = c(rep("NT_GFP",3), rep("PA_GFP",2))

data.filter <- data.filter %>% select("X8988t.NT1.mCherry","X8988t.NT4.mCherry","X8988t.NT5.mCherry",
"X8988t.PA1.mCherry","X8988t.PA2.mCherry","X8988t.PA3.mCherry")
group_info = c(rep("NT_mCherry",3), rep("PA_mCherry",3))

data.filter <- data.filter %>% select("X8988t.NT1.GFP","X8988t.NT3.GFP","X8988t.NT4.GFP",
"X8988t.NT1.mCherry","X8988t.NT4.mCherry","X8988t.NT5.mCherry")
group_info = c(rep("NT_GFP",3), rep("NT_mCherry",3))
...
#2.  exactTest
dge.list.res <- exactTest(dge.list.obj, pair = c("NT_GFP","NT_mCherry"))
#dge.list.res <- exactTest(dge.list.obj, pair = c("NT_GFP","PA_GFP"))
#dge.list.res <- exactTest(dge.list.obj, pair = c("NT_mCherry","PA_mCherry"))
#dge.list.res <- exactTest(dge.list.obj, pair = c("PA_GFP","PA_mCherry"))
...
write.table(normalizeExp,file="Part1_normalizeExp.xls",sep="\t",quote=F,col.names=T)   #输出所有基因校正后的表达值（normalizeExp.txt）

#DEGs normalized counts table--diffExp
diffExp=normalizeExp[diff_nameSig$ensembl_gene_id,]
write.table(diffExp,file="Part1_diffmRNAExp.xls",sep="\t",quote=F,col.names=T)         #输出差异基因校正后的表达值（diffmRNAExp.txt）
nrow(diffExp)
